...
module.exports = function (app, options, multitenancy) {
    ...
    passport.use(new BasicStrategy(function (username, password, done) {
        ...
        multitenancy.multitenancyRepository.authenticate(username, password).then(function (tenant) {
            ...
            multitenancy.activateTenant(tenant, function (err, tenant) {
                if (err) {
                    return done(err);
                }
    ...
    passport.deserializeUser(function (id, done) {
        multitenancy.multitenancyRepository.findTenant(id).then(function(tenant) {
            if (!tenant) {
                logger.debug("tenant not found " + id);
                return done({
            ...
            logger.debug("authenticated user from cookie " + tenant.name);
            done(null, tenant);
        }).catch(function(e) {
            logger.warn("error when searching for user " + username + " " + e.stac);
            return done(e);
        });
    });
    ...
    app.get("/", function (req, res, next) {
        if (!req.user) {
            logger.debug("request without valid credentials");
    ...
    app.post('/login', function (req, res, next) {
        ...
        passport.authenticate('local', function (err, user, info) {
            ...
            logger.debug("login successfull " + JSON.stringify(user));
...
//authenticate basic if request to API
    ...
    app.use(function (req, res, next) {
        ...
        multitenancy.activateTenant(req.user, function (err, tenant) {
            if (err) {
                return next(err);
            }

            if (!options.useSubDomains || options.subdomainsCount == domains.length) {
                logger.debug("routing request to vhost");
                //route request to tenant
                return tenant.reporter.options.express.app(req, res, next);
            }

            logger.debug("redirecting back " + domains.length + " != " + options.subdomainsCount);

            //add tenant subdomain and route to root page
            domains.unshift(tenant.name);

            logger.debug("redirecting to " + "https://" + domains.join("."));
            return res.redirect("https://" + domains.join("."));
        });
    });
