...
module.exports = function (app, options, cb) {
    ...
    function activateTenant(tenant, tcb) {
        ...
        if (!fs.existsSync(path.join(opts.rootDirectory, 'logs'))) {
            fs.mkdir(path.join(opts.rootDirectory, 'logs'));
        }

        if (!winston.loggers.has('jsreport-' + tenant.name)) {
            var consoleTransport = new (winston.transports.Console)(transportSettings);
            var fileTransport = new (winston.transports.File)({ name: "main", filename: path.join(opts.rootDirectory, 'logs', 'reporter.log'), maxsize: 10485760, json: false, level: "debug" });
            var errorFileTransport = new (winston.transports.File)({ name: "error", level: 'error', filename: path.join(opts.rootDirectory, 'logs', 'error.log'), handleExceptions: true, json: false });

            opts.logger = winston.loggers.add('jsreport-' + tenant.name, {
                transports: [consoleTransport, fileTransport, errorFileTransport]
            });

            var methods = ["info", "warn", "error", "debug"];
            methods.forEach(function (m) {
                var originalMethod = opts.logger[m];
                opts.logger[m] = function (message) {
                    originalMethod(tenant.name + " " + message);
                };
            });
        }

        opts.logger = winston.loggers.get('jsreport-' + tenant.name);
